...
$stmt = $pdo->prepare("
    ...
    (SELECT COUNT(DISTINCT user_id) FROM tickets WHERE game_id = raffle_games.id AND payment_verified = TRUE) AS player_count,  -- Count unique players
...
if (isset($_GET['stop_game'])) {
    ...
    // Ensure only the game creator can stop the game
...
<table class="table table-bordered">
    ...
    <?php foreach ($games as $game): 
        ...
        $endType = $game['timed_event'] == 1 ? 'Timed' : 'Manual'; // Determine if the game is timed or manual
    ?>
        <tr>
            ...
            <td>
                ...
                <!-- Add Stop Game button only for the creator and non-timed games -->
                <?php if ($game['timed_event'] == 0 && $game['status'] == 'active' && $game['user_id'] == $user_id): ?>
